...
def write_tests(output_filename, siteTests):
    ...
    for test in siteTests:
        ...
        c.execute('INSERT INTO sitetests (site_id, test_date, type_of_test, check_report, json_check_data, most_recent, rating) VALUES (?, ?, ?, ?, ?, ?, ?);\n', (test["site_id"], test["date"], test["type_of_test"], test["report"], test["data"], 1, test["rating"]))
        conn.commit()
