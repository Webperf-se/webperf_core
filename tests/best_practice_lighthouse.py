...
def run_test(global_translation, lang_code, url, strategy='mobile', category='best-practices'):
    ...
    if points >= 5.0:
        review = local_translation("TEXT_REVIEW_PRACTICE_VERY_GOOD") + review
    elif points >= 4.0:
        review = local_translation("TEXT_REVIEW_PRACTICE_IS_GOOD") + review
    elif points >= 3.0:
        review = local_translation("TEXT_REVIEW_PRACTICE_IS_OK") + review
    elif points > 1.0:
        review = local_translation("TEXT_REVIEW_PRACTICE_IS_BAD") + review
    elif points <= 1.0:
        review = local_translation("TEXT_REVIEW_PRACTICE_IS_VERY_BAD") + review
